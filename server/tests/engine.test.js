...
test('initial setup contains six non-overlapping pieces for both players', () => { const g = game(); for (const owner of ['one', 'two']) assert.deepEqual(g.board.filter((t) => t.owner === owner).map((t) => t.type), E.TILE_TYPES); assert.equal(new Set(g.board.map((tile) => tile.position)).size, 12); });
test('initial sides are mirrored and use the canonical Avatar/Lotus starts', () => { const g = game(); for (const type of E.TILE_TYPES) { const a = piece(g, 'one', type); const b = piece(g, 'two', type); const [ax, ay] = a.position.split(',').map(Number); assert.equal(b.position, `${-ax},${-ay}`); assert.equal(a.startPosition, a.position); } assert.equal(piece(g, 'one', 'avatar').position, '0,-2'); assert.equal(piece(g, 'one', 'lotus').position, '0,-4'); assert.equal(piece(g, 'two', 'avatar').position, '0,2'); assert.equal(piece(g, 'two', 'lotus').position, '0,4'); });
...
test('two clients receive the same serializable authoritative state', () => { const saved = E.applyMove(game(), 'one', command('one-lotus', '1,-4')); const clientA = structuredClone(saved); const clientB = structuredClone(saved); assert.deepEqual(clientA, clientB); assert.deepEqual(clientA.legalMoves, clientB.legalMoves); });
