...
test('ordinary steps exclude occupied and off-board cells', () => {
  ...
  const centerTargets = E.legalTargets(center, 'one-water-1');
  for (const target of ['1,0', '-1,0', '0,1', '0,-1', '1,-1', '-1,1']) assert.ok(centerTargets.includes(target));
  assert.ok(!centerTargets.includes('1,1'));
  assert.ok(!centerTargets.includes('-1,-1'));
...
test('friendly jumps work in each of the eight directions for both players', () => {
...
test('a jump chain can change direction and records every landing', () => {
  const g = sparse([['one', 'water', '0,0'], ['one', 'earth', '1,1'], ['one', 'fire', '3,2']]);
  const moved = E.applyMove(g, 'one', command('one-water-1', '4,2'));
  assert.deepEqual(moved.lastMove.route, ['0,0', '2,2', '4,2']);
});
test('jumps cannot land on an occupied cell or beyond the board', () => {
  const occupied = sparse([['one', 'water', '0,0'], ['one', 'earth', '1,1'], ['two', 'fire', '2,2']]);
  assert.ok(!E.legalTargets(occupied, 'one-water-1').includes('2,2'));
  const edge = sparse([['one', 'water', '1,-9'], ['one', 'earth', '2,-9']]);
  assert.ok(!E.cellMap.has('3,-9'));
  assert.ok(!E.legalTargets(edge, 'one-water-1').includes('3,-9'));
});
test('jump routes do not cycle or revisit the vacated starting cell', () => {
  const g = sparse([['one', 'water', '0,0'], ['one', 'earth', '1,0'], ['one', 'fire', '2,1'], ['one', 'air', '1,2'], ['one', 'avatar', '0,1']]);
  assert.ok(!E.legalTargets(g, 'one-water-1').includes('0,0'));
...
test('both clients receive the same diagonal chain endpoint and animation route', () => {
  const g = sparse([['one', 'water', '0,0'], ['one', 'earth', '1,1'], ['one', 'fire', '3,2']]);
  const saved = E.applyMove(g, 'one', command('one-water-1', '4,2'));
  const clientA = structuredClone(saved); const clientB = structuredClone(saved);
  assert.equal(piece(clientA, 'one', 'water').position, '4,2');
  assert.equal(piece(clientB, 'one', 'water').position, '4,2');
  assert.deepEqual(clientA.lastMove.route, clientB.lastMove.route);
});
