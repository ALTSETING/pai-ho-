...
// Combat adjacency and jumps keep their original side-sharing directions.
// Ordinary steps additionally allow cells that share only a diamond vertex.
const COMBAT_DIRECTIONS = Object.freeze([[1, 0], [-1, 0], [0, 1], [0, -1]]);
const STEP_DIRECTIONS = Object.freeze([...COMBAT_DIRECTIONS, [1, 1], [1, -1], [-1, 1], [-1, -1]]);
function adjacentIds(id) {
  const cell = cellMap.get(id);
  if (!cell) return [];
  return COMBAT_DIRECTIONS.map(([dx, dy]) => `${cell.column + dx},${cell.row + dy}`).filter((candidate) => cellMap.has(candidate));
}
function stepIds(id) {
  const cell = cellMap.get(id);
  if (!cell) return [];
  return STEP_DIRECTIONS.map(([dx, dy]) => `${cell.column + dx},${cell.row + dy}`).filter((candidate) => cellMap.has(candidate));
}
