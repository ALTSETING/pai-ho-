...
function jumpTargets(state, tile) {
  ...
  function visit(cell) {
    for (const [dx, dy] of JUMP_DIRECTIONS) {
      ...
      // The moving tile no longer occupies its starting cell while exploring
      // the route. It therefore cannot serve as a hurdle or block a landing.
      const middle = tileAt(state, middleId); const landingTile = tileAt(state, landingId);
      if (!cellMap.has(landingId) || (landingTile && landingTile.id !== tile.id) || !middle || middle.id === tile.id || middle.owner !== tile.owner || visited.has(landingId)) continue;
...
function jumpRoute(state, tile, target) {
  ...
  const search = (cell, route) => {
    ...
    for (const [dx, dy] of JUMP_DIRECTIONS) {
      const middleId = `${cell.x + dx},${cell.y + dy}`; const landingId = `${cell.x + 2 * dx},${cell.y + 2 * dy}`;
      const middle = tileAt(state, middleId); const landing = cellMap.get(landingId); const landingTile = tileAt(state, landingId);
      if (!landing || (landingTile && landingTile.id !== tile.id) || !middle || middle.id === tile.id || middle.owner !== tile.owner || visited.has(landingId)) continue;
